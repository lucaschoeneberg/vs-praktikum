...
public class BillBoardHtmlAdapter extends BillBoard implements BillBoardAdapterIf {
    ...
    public List<String> readEntriesList(String caller_ip) {
        List<String> result = new ArrayList<>();
        for (BillBoardEntry e : billboard) {
            result.add(readEntry(e.id, caller_ip));
        }
        return result;
    }

    public Map<String, String> readEntriesMap(String caller_ip) {
        Map<String, String> result = new HashMap<>();
        for (BillBoardEntry e : billboard) {
            result.put(String.valueOf(e.id), readEntry(e.id, caller_ip));
        }
        return result;
    }

    public Map<Integer, String> readEntriesList() {
        Map<Integer, String> result = new HashMap<>();
        for (BillBoardEntry e : billboard) {
            result.put(e.id, e.text);
        }
        return result;
    }
